...
for i=1:numel(data_dirs)
    try
        ...
        disp({['Path for stacks as *.mat files: ' dirs.mat];...
            ['Path for info file: ' path.regData]});
        ...
        options.seed = NoRMCorreSetParms('d1',stackInfo.ImageLength,'d2',stackInfo.ImageWidth,'max_shift',[15,15],...
            'upd_template',false,'use_parallel',true,'output_type','mat'); %Initial rigid correct for drift
        options.RMC = NoRMCorreSetParms('d1',stackInfo.ImageLength,'d2',stackInfo.ImageWidth,'max_shift',[10,10],...
            'upd_template',false,'use_parallel',true,'output_type','mat'); %Rigid Correct; avg whole stack for template
        options.NRMC = NoRMCorreSetParms('d1',stackInfo.ImageLength,'d2',stackInfo.ImageWidth,...
            'grid_size',[64,64],'overlap_pre',[16,16],'overlap_post',[16,16],'us_fac',20,'mot_uf',4,...
            'max_shift',[10,10],'max_dev',[5,5],'upd_template',false,'use_parallel',true,...
        ...
        for m = find(params.max_reps) %seed, rigid, non-rigid
            ...
            [template,nReps,err_mat.(options_label{m})] = ...
                iCorre(path.mat,options.(options_label{m}),options_label{m},template,...
            ...
            save(path.regData,'template_out','err_mat','nRepeats','run_times','-append'); %save values
        ...
        save(path.regData,'options','-append');
        ...
        if params.split_channels.ref_channel~=params.split_channels.reg_channel
            ...
            for k = 1:numel(file_names)
                S = load(path.mat{k},'stack'); %load into struct to avoid eval(stack_names{k})
        ...
        save(path.regData,'run_times','-append'); %save parameters
        
        clearvars '-except' root_dir data_dirs file_names dirs path params stackInfo options_label run_times status msg i m;
    ...
    if ~exist('err','var')
        ...
        status(i) = false;
        err_msg{i} = err;
        save(path.regData,'err_msg','-append'); %save error msg
    end
