function paths = applyShifts_multiChannel(paths, dirs, stackInfo, params)
...
%Initialize outputs and create subdirectories
[~,fnames,~] = fileparts(paths.raw);
prefix = repmat(reg_params(end),numel(fnames),1);
if params.save_interleaved
    dirs.save_tiff = fullfile(dirs.main);
end
if params.preserve_chans
    path_save = fullfile(dirs.main,'registered-interleaved', join([prefix,fnames],'_'));
else
    for i = 1:numel(chan_ID)
        path_save{i} = fullfile(dirs.main,['registered-chan' num2str(chan_ID(i))], join([prefix,fnames],'_'));
    end
...
parfor i = 1:numel(paths.mat)
iCorreApplyShifts(path_source{i}, mat_path{i}, path_save(i,:), reg_params, chan_ID, crop_margins, save_interleaved);
